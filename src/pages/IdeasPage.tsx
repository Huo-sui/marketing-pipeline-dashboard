...
export function IdeasPage() {
  ...
  const openTopic = () => {
    topicForm.setFieldsValue({ title: "", hook: "", format: "图文", copy: "", sourcePostIds: approvedSources.slice(0, 1).map((source) => source.id), platforms: ["小红书"], assetMatch: "pending", imageBrief: "" });
    setTopicOpen(true);
  };
  ...
  const copySkillPrompt = async () => {
    await navigator.clipboard.writeText("请使用 $viral-topic-analysis 分析当前项目已通过的爆帖，结合项目方向与资产，生成可终审选题，并把痛点、反馈和灵感保存到灵感箱。不要自动批准、生成媒体或发布。");
  ...
  return <>{contextHolder}
    <PageHeader title="选题箱" meta="爆帖证据转化为可编辑选题；所有内容都要经过人工终审" actions={<Space wrap><Button icon={<ClipboardCopy size={15} />} onClick={copySkillPrompt}>复制 Codex 分析指令</Button><Button icon={<CheckCheck size={15} />} disabled={selected.length === 0} onClick={approveSelected}>批量通过</Button><Button type="primary" icon={<Plus size={15} />} disabled={approvedSources.length === 0} onClick={openTopic}>新增人工选题</Button></Space>} />
    <Alert className="section-alert" type="info" showIcon message={<span>当前有 <strong>{approvedSources.length}</strong> 条已通过来源帖。仓库级 <code>$viral-topic-analysis</code> Skill 会结合项目方向、项目资产和真实证据生成选题，但不会越过人工终审。</span>} />
    <div className="idea-config-strip"><div><span className="muted">来源门槛</span><strong>已通过爆帖</strong></div><div><span className="muted">图片策略</span><strong>项目资产 / 结构化生图</strong></div><div><span className="muted">视频</span><strong>仅预留生成位</strong></div><div><span className="muted">项目资产</span><Link to="/assets">查看资产库</Link></div></div>
