import type { AccountRecord, AccountSetupInput, AssetRecord, AutoGlmProviderStatus, AutoGlmProviderTest, ContentDraftRecord, GenerationJob, IdeaRecord, IdeaStatus, InsightRecord, ProjectAccountBinding, ProjectRecord, ProjectSetupInput, ProjectStatus, RunEventRecord, RunRecord, SourcePost, TopicPreflightResult, TopicWatch, TopicWatchInput, ViralPostAnalysis } from "../types";
...
export type PlatformAdapterStatus = { platform: string; id: string; label: string; configured: boolean; mode: "phone" | "http"; capabilities: Record<string, boolean>; requirements: Record<string, boolean>; extraction: { requiredFields: string[] }; discovery: { mediaTypes: Array<"any" | "video" | "image_text">; stateMachineVersion: string; defaultCandidateLimit: 3 }; detail: string };
...
function mapSourcePost(post: Record<string, unknown>): SourcePost {
  ...
  const sourceLinkValue = post.sourceLink && typeof post.sourceLink === "object" && !Array.isArray(post.sourceLink) ? post.sourceLink as Record<string, unknown> : {};
  const sourceLink = { usable: sourceLinkValue.usable !== false, ...(typeof sourceLinkValue.reason === "string" ? { reason: sourceLinkValue.reason } : {}) };
  return { id: String(post.id), platform: post.platform as SourcePost["platform"], author: String(post.author || ""), title: String(post.title || ""), body: String(post.body || ""), topic: String(post.topic || ""), published: String(post.publishedAt || post.published || "待确认"), likes: Number(post.likes || 0), comments: Number(post.comments || 0), score: Number(post.score || 0), reason: String(post.reason || "未匹配话题规则"), mediaType: (post.mediaType || "文本") as SourcePost["mediaType"], image: String(post.image || ""), canonicalUrl: String(post.canonicalUrl || ""), sourceLink, externalId: String(post.externalId || ""), capturedAt: String(post.capturedAt || ""), patternCard, patternCardVersion: typeof post.patternCardVersion === "number" ? post.patternCardVersion : undefined, evidence: { signals, scoreBreakdown, capturedBy: String(evidence.capturedBy || post.capturedBy || "control-api"), capturedAt: String(evidence.capturedAt || post.capturedAt || "") } };
}
